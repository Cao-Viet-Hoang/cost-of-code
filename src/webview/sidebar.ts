 ...
 * inside both the light and the dark lightness band. Three independent sets are
 * in play, each validated on its own because they never share a chart — the two
 * tool hues (stacked share bar), the three donut slots (top-model donut, which
 * needs all-pairs separation, not just adjacent), and the single accent used by
 * the trend line and the meter. Contrast against the light surface is in the
 * relief band, which is why every mark here carries a visible label and value.
 */
...
/* ---------- stacked share bar ---------- */
/* Rides inside the Today tile with no heading of its own: the legend already
   says Claude and Codex, and a heading would cost a section's worth of height. */
.share { margin-top: 10px; }
.share[hidden] { display: none; }
/* Segments are separated by a 2px surface gap, never a border. */
.stack { display: flex; gap: 2px; height: 10px; }
.stack .seg { border-radius: 1px; min-width: 2px; }
.stack .seg:first-child { border-radius: 4px 1px 1px 4px; }
.stack .seg:last-child  { border-radius: 1px 4px 4px 1px; }
.stack .seg[data-tool="claude"] { background: var(--tool-claude); }
.stack .seg[data-tool="codex"]  { background: var(--tool-codex); }

.legend { list-style: none; margin: 6px 0 0; padding: 0; display: flex; flex-wrap: wrap; gap: 4px 14px; font-size: 0.92em; }
.legend li { display: flex; align-items: center; gap: 5px; }
.legend .swatch { width: 8px; height: 8px; border-radius: 2px; flex: 0 0 auto; }
.legend .swatch[data-tool="claude"] { background: var(--tool-claude); }
.legend .swatch[data-tool="codex"]  { background: var(--tool-codex); }
.legend .name { color: var(--vscode-descriptionForeground); }
...
<section class="block tile">
  ...
  <div class="share" id="splitRow" hidden>
    <div class="stack" id="splitBar"></div>
    <ul class="legend" id="splitLegend"></ul>
  </div>
...
const toolName = (t) => (t === 'codex' ? 'Codex' : 'Claude');
...
function renderStatus(d) {
  ...
  if (!d.claudeConfigured && d.codexEnabled && d.codexRecords) {
    // Codex-only user: the Claude collector is irrelevant, do not alarm them.
    state = 'ok';
    label = 'Codex sessions only';
    tip = 'Reading ~/.codex/sessions. No collector needed for Codex.';
  }
...
function renderSplit(d) {
  const block = el('splitRow');
  const split = d.todaySplit || {};
  const parts = ['claude', 'codex']
    .map((t) => ({ tool: t, cost: (split[t] || {}).cost || 0, requests: (split[t] || {}).requests || 0 }))
    .filter((p) => p.cost > 0 || p.requests > 0);
  // A single tool is not a part-to-whole story — the tile above already says it.
  if (parts.length < 2) { block.hidden = true; return; }
  block.hidden = false;

  const total = parts.reduce((s, p) => s + p.cost, 0) || 1;
  const bar = el('splitBar');
  bar.innerHTML = parts.map((p) =>
    '<span class="seg" data-tool="' + p.tool + '" style="flex:' + Math.max(0.02, p.cost / total).toFixed(4) + '"></span>',
  ).join('');
  Array.prototype.forEach.call(bar.children, (seg, i) => {
    const p = parts[i];
    bindTip(seg,
      '<div class="t">' + toolName(p.tool) + '</div>' +
      '<div class="v">' + fmtCostExact(p.cost) + ' · ' + ((p.cost / total) * 100).toFixed(0) + '% of today</div>');
  });

  el('splitLegend').innerHTML = parts.map((p) =>
    '<li><span class="swatch" data-tool="' + p.tool + '"></span>' +
    '<span class="name">' + toolName(p.tool) + '</span>' +
    '<span class="val">' + fmtCost(p.cost) + '</span></li>',
  ).join('');
}

/** Lucide-style SVG icon strings, used both inline in HTML and from JS. */
export const ICONS = {
  ...
  trend:       '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="M18 17V9"/><path d="M13 17V5"/><path d="M8 17v-3"/></svg>',
  ...
  activity:    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 12h-2.48a2 2 0 0 0-1.93 1.46l-2.35 8.36a.5.5 0 0 1-.96 0L8.59 2.18a.5.5 0 0 0-.96 0L5.28 10.54A2 2 0 0 1 3.35 12H2"/></svg>',
  refresh:     '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M3 21v-5h5"/></svg>',
...
/** Same map, encoded for inclusion in the inline JS bundle. */
export function buildIconsJs(): string {
  const entries = Object.entries(ICONS).map(([k, v]) => `${k}: ${JSON.stringify(v)}`);
  return `const ICONS = { ${entries.join(', ')} };`;
}

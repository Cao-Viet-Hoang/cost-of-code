...
import type { AggregatedTotals, FilterOptions } from './types';
...
interface SidebarPayload {
  ...
  /**
   * Newest usage activity. This is what the status stamp shows —
   * `updatedAt` below is stamped at read time, so rendering it as a relative
   * time always says "0s ago" and measures nothing.
   */
...
export class SidebarView implements vscode.WebviewViewProvider {
  ...
  constructor(private readonly extensionUri: vscode.Uri) {
    this.disposables.push(
      vscode.workspace.onDidChangeConfiguration((e) => {
        ...
        // Full HTML reload rather than a targeted update — some settings
        // (autoRefreshSeconds) are baked into buildSidebarJs at render time,
        // so there is no smaller invalidation than re-emitting the page.
        if (this.view) {
  ...
  async refresh() {
    ...
    try {
      ...
      const payload: SidebarPayload = {
        ...
        lastActivityAt: health.lastUsageAt,
  ...
  private rankedModels(reader: UsageReader, filter: FilterOptions): TopModel[] {
    return reader.models(filter).map((m) => ({ model: m.model, cost: m.cost }));
  }
